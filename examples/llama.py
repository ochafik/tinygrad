...
class LLaMa:
  ...
  def build(model_path, tokenizer_path, model_gen=1, model_size="7B", quantize=False):
    ...
    if re.search(r'.*?\.(bin|safetensors|index\.json)$', model_path):
...
if __name__ == "__main__":
  ...
  parser.add_argument('--model', type=str, default=None, help="The weights file to load")
...
"""
  ...
  WEIGHTS_DIR = Path(__file__).parent.parent / f"weights/LLaMA{LLAMA_SUFFIX}/" \
    if args.model is None else args.model if Path(args.model).is_dir() else Path(args.model).parent
  TOKENIZER_FILENAME = WEIGHTS_DIR / "tokenizer.model"
  print(f"using LLaMA{LLAMA_SUFFIX}-{args.size} model")
  llama = LLaMa.build(args.model or f'{WEIGHTS_DIR}/{args.size}', TOKENIZER_FILENAME, model_gen=args.gen, model_size=args.size, quantize=args.quantize)
