...
def load_state_dict(model, state_dict, strict=True):
  if 'tok_embeddings.weight' in state_dict:
    hf_keymap = {
      ...
      **{f'layers.{l}.attention_norm.weight'   : f'model.layers.{l}.input_layernorm.weight' for l in range(len(model.layers))},
      **{f'layers.{l}.attention.w{x}.weight'   : f'model.layers.{l}.self_attn.{x}_proj.weight' for x in ['q', 'k', 'v', 'o'] for l in range(len(model.layers))},
      **{f'layers.{l}.ffn_norm.weight'         : f'model.layers.{l}.post_attention_layernorm.weight' for l in range(len(model.layers))},
      **{f'layers.{l}.feed_forward.w{x}.weight': f'model.layers.{l}_proj.mlp.{y}.weight' for x, y in {'1': 'gate', '2': 'down', '3': 'up'}.items() for l in range(len(model.layers))},
      'norm.weight': 'model.norm.weight',
      'output.weight': 'lm_head.weight',
    }
    state_dict = {hf_keymap[k]: v for k,v in state_dict.items() if k != 'rope.freqs'}
